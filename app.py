...
data = load_json(DATA_FILE, {
    "users": [], "groups": [], "tasbih": {},
    "last_reset": {},
    "used_ramadan_duaa": {}, "tasbih_sessions": {}, "group_settings": {}
})
...
# إعدادات كل قروب: العدد ونوع المحتوى
group_settings    = data.get("group_settings", {})

# عداد الرسائل لكل قروب — كل 10 رسائل يرسل ذكر أو فضل
group_msg_counter = {}
...
def is_valid_command(text):
    ...
    if normalize_tasbih(t):
        return True
    # أمر ضبط العدد: "ضبط 15"
    if t.startswith("ضبط ") and t[4:].strip().isdigit():
        return True
    return False
...
HELP_TEXT = (
    ...
    "─────────────────\n"
    "التسبيح\n"
    "تسبيح ← عداد فردي لك وحدك\n"
    "تسبيح مشترك ← تفتح جلسة مشتركة\n"
...
def handle_message(event):
    try:
        ...
        if gid:
            ...
            mode      = settings.get("mode", "الكل")
            if limit == 0:
                pass
            else:
                group_msg_counter[gid] = group_msg_counter.get(gid, 0) + 1
            if limit > 0 and group_msg_counter[gid] >= limit:
                group_msg_counter[gid] = 0
                if mode == "ذكرني":
                    pool = content.get("adhkar", [])
                elif mode == "فضل":
                    pool = fadl_content
                else:
                    pool = content.get("adhkar", []) + fadl_content
                if pool:
                    reply_message(event.reply_token, random.choice(pool))
                    return
        ...
        # ضبط عدد الرسائل — "ضبط 15"
        if cmd.startswith("ضبط ") and cmd[4:].strip().isdigit():
            if not gid:
                reply_message(event.reply_token, "هذا الأمر للقروبات فقط.")
                return
            num = int(cmd[4:].strip())
            if gid not in group_settings:
                group_settings[gid] = {}
            group_settings[gid]["limit"] = num
            save_data()
            if num == 0:
        ...
        # تخصيص نوع المحتوى
        if cmd in ("محتوى ذكرني", "محتوى فضل", "محتوى الكل"):
            if not gid:
                reply_message(event.reply_token, "هذا الأمر للقروبات فقط.")
                return
            mode_map = {"محتوى ذكرني": "ذكرني", "محتوى فضل": "فضل", "محتوى الكل": "الكل"}
            mode = mode_map[cmd]
            if gid not in group_settings:
                group_settings[gid] = {}
        ...
        if cmd == "ذكرني":
            ...
            def do_broadcast():
                ...
                logger.info(f"ذكرني: أُرسل إلى {sent} قروب")
