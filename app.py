...
def handle_message(event):
    try:
        ...
        # إيقاف / تشغيل
        if cmd == "إيقاف":
            tid = gid if gid else user_id
            if tid not in notifications_off:
                notifications_off.add(tid)
                save_data()
                reply_message(event.reply_token,
                    "تم إيقاف التذكيرات لهذا القروب." if gid else "تم إيقاف التذكيرات لك.")
            else:
                reply_message(event.reply_token, "التذكيرات موقوفة مسبقاً.")
            return

        if cmd == "تشغيل":
            tid = gid if gid else user_id
            if tid in notifications_off:
                notifications_off.discard(tid)
                save_data()
                reply_message(event.reply_token,
                    "تم تشغيل التذكيرات لهذا القروب." if gid else "تم تشغيل التذكيرات لك.")
            else:
                reply_message(event.reply_token, "التذكيرات تعمل مسبقاً.")
            return
        ...
        if cmd == "احصائيات":
            ...
            stats_text = (
                "إحصائيات البوت\n"
                "─────────────────\n"
                f"المجموعات   : {len(target_groups)}\n"
                f"المستخدمون  : {len(target_users)}\n"
                f"موقوف       : {len(notifications_off)}\n"
                f"التسبيحات   : {total_tasbih}\n"
                f"الأذكار     : {len(content.get('adhkar', []))}\n"
                f"الفضائل     : {len(fadl_content)}\n"
                f"أدعية رمضان : {len(ramadan_duaa_list)}\n"
                f"جلسات مشتركة: {len(tasbih_sessions)}\n"
                "─────────────────\n"
                "تم إنشاء هذا البوت بواسطة عبير الدوسري"
            )
        ...
        if cmd == "مشترك":
            ...
            if open_sid:
                ...
                reply_message(event.reply_token,
                    "لا توجد جلسة مشتركة نشطة الآن.\n"
                    "لبدء جلسة اكتب: تسبيح مشترك")
            return
        ...
        if cmd == "ذكرني":
            ...
            def do_broadcast():
                ...
                for g in list(target_groups):
                    if g != gid and g not in notifications_off:
...
def stats():
    ...
    return jsonify({
        ...
        "notifications_off": len(notifications_off),
    ...
    try:
        if fadl_content:
            ...
            sent = sum(1 for g in list(target_groups)
                       if g not in notifications_off and send_message(g, message))
